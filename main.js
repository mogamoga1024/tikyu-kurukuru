...
function 描画() {
    ...
    for (let i = 0; i < 世界地図行List.length; i++) {
        ...
        console.log((宇宙空間 + " " + 歪んだ世界地図の一部 + " " + 宇宙空間 + "#").slice(0, 最大幅 + 2));
    }
}

function 歪んだ世界地図行を作る(世界地図行, 円行文字数) {
    const けずる文字数 = Math.floor((最大幅 - 円行文字数) / 2);
    let 世界地図行2 = 世界地図行.slice(けずる文字数, -けずる文字数);
    世界地図行2 = 世界地図行.slice(0, 1) + 世界地図行.slice(2, -2) + 世界地図行.slice(-1);

    let カウントしている文字 = 世界地図行2[0];
    let count = 0;
    const rle = [];
    for (const 世界地図行の文字 of 世界地図行2) {
